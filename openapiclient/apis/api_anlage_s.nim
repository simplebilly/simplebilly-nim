...
import ../models/model_anlage_s_ergebnis
...
proc anlageSApi*(httpClient: HttpClient, year: int): (Option[AnlageSErgebnis], Response) =
  ## 
  var query_params_list: seq[(string, string)] = @[]
  query_params_list.add(("year", $year))
  let url_encoded_query_params = encodeQuery(query_params_list)

  let response = httpClient.get(basepath & "/api/v1/bookkeeping/anlage-s" & "?" & url_encoded_query_params)
  constructResult[AnlageSErgebnis](response)
