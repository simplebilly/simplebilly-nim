...
import ../models/model_kst_ergebnis
...
proc kstApi*(httpClient: HttpClient, year: int, gewinn: string): (Option[KstErgebnis], Response) =
  ## 
  var query_params_list: seq[(string, string)] = @[]
  query_params_list.add(("year", $year))
  if $gewinn != "":
    query_params_list.add(("gewinn", $gewinn))
  let url_encoded_query_params = encodeQuery(query_params_list)

  let response = httpClient.get(basepath & "/api/v1/bookkeeping/kst" & "?" & url_encoded_query_params)
  constructResult[KstErgebnis](response)
