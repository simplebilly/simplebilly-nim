...
proc globalSearch*(httpClient: HttpClient, q: string): (Option[JsonNode], Response) =
  ## GET /api/v1/search?q=...
  var query_params_list: seq[(string, string)] = @[]
  query_params_list.add(("q", $q))
  let url_encoded_query_params = encodeQuery(query_params_list)

  let response = httpClient.get(basepath & "/api/v1/search" & "?" & url_encoded_query_params)
  constructResult[JsonNode](response)


proc myPermissions*(httpClient: HttpClient): (Option[JsonNode], Response) =
  ## GET /api/v1/me/permissions — resolved permissions from the auth token, used by the frontend to show/hide admin navigation.

  let response = httpClient.get(basepath & "/api/v1/me/permissions")
  constructResult[JsonNode](response)
