#
# SimpleBilly API
# 
# Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
# The version of the OpenAPI document: 0.1.0
# Contact: support@simplebilly.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options


type AbsenceStatus* {.pure.} = enum
  Pending
  Approved
  Rejected
  Cancelled

func `%`*(v: AbsenceStatus): JsonNode =
  result = case v:
    of AbsenceStatus.Pending: %"pending"
    of AbsenceStatus.Approved: %"approved"
    of AbsenceStatus.Rejected: %"rejected"
    of AbsenceStatus.Cancelled: %"cancelled"

func `$`*(v: AbsenceStatus): string =
  result = case v:
    of AbsenceStatus.Pending: $("pending")
    of AbsenceStatus.Approved: $("approved")
    of AbsenceStatus.Rejected: $("rejected")
    of AbsenceStatus.Cancelled: $("cancelled")

proc to*(node: JsonNode, T: typedesc[AbsenceStatus]): AbsenceStatus =
  if node.kind != JString:
    raise newException(ValueError, "Expected string for enum AbsenceStatus, got " & $node.kind)
  let strVal = node.getStr()
  case strVal:
  of $("pending"):
    return AbsenceStatus.Pending
  of $("approved"):
    return AbsenceStatus.Approved
  of $("rejected"):
    return AbsenceStatus.Rejected
  of $("cancelled"):
    return AbsenceStatus.Cancelled
  else:
    raise newException(ValueError, "Invalid enum value for AbsenceStatus: " & strVal)

