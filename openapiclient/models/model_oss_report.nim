#
# SimpleBilly API
# 
# Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
# The version of the OpenAPI document: 0.1.0
# Contact: support@simplebilly.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options

import model_oss_dependency

type OssReport* = object
  ## 
  dependencies*: seq[OssDependency]
  totalCount*: int


# Custom JSON deserialization for OssReport with custom field names
proc to*(node: JsonNode, T: typedesc[OssReport]): OssReport =
  result = OssReport()
  if node.kind == JObject:
    if node.hasKey("dependencies"):
      # Array of types with custom JSON - manually iterate and deserialize
      let arrayNode = node["dependencies"]
      if arrayNode.kind == JArray:
        result.dependencies = @[]
        for item in arrayNode.items:
          result.dependencies.add(to(item, OssDependency))
    if node.hasKey("total_count"):
      result.totalCount = to(node["total_count"], int)

# Custom JSON serialization for OssReport with custom field names
proc `%`*(obj: OssReport): JsonNode =
  result = newJObject()
  result["dependencies"] = %obj.dependencies
  result["total_count"] = %obj.totalCount

