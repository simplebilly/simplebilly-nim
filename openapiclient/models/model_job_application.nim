#
# SimpleBilly API
# 
# Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
# The version of the OpenAPI document: 0.1.0
# Contact: support@simplebilly.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options

import model_application_status

type JobApplication* = object
  ## 
  cvFile*: Option[string] ## Relative path of the stored CV file under the upload dir.
  cvText*: Option[string] ## Extracted CV text, used for match-scoring.
  email*: Option[string]
  matchReason*: Option[string]
  matchScore*: Option[int] ## 0-100 LLM match score against the posting's required profile.
  name*: Option[string]
  phone*: Option[string]
  postingId*: Option[string] ## References the job_posting entity.
  source*: string ## website | email | board
  status*: ApplicationStatus ## new | reviewing | interview | hired | rejected

