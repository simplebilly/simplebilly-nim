...
type SmtpEncryption* {.pure.} = enum
  StartTls
  Tls

func `%`*(v: SmtpEncryption): JsonNode =
  result = case v:
    of SmtpEncryption.StartTls: %"StartTls"
    of SmtpEncryption.Tls: %"Tls"

func `$`*(v: SmtpEncryption): string =
  result = case v:
    of SmtpEncryption.StartTls: $("StartTls")
    of SmtpEncryption.Tls: $("Tls")

proc to*(node: JsonNode, T: typedesc[SmtpEncryption]): SmtpEncryption =
  if node.kind != JString:
    raise newException(ValueError, "Expected string for enum SmtpEncryption, got " & $node.kind)
  ...
  of $("Tls"):
    ...
    raise newException(ValueError, "Invalid enum value for SmtpEncryption: " & strVal)
