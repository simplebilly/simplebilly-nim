...
# Custom JSON deserialization for XRechnungResponse with custom field names
proc to*(node: JsonNode, T: typedesc[XRechnungResponse]): XRechnungResponse =
  result = XRechnungResponse()
...
# Custom JSON serialization for XRechnungResponse with custom field names
proc `%`*(obj: XRechnungResponse): JsonNode =
  result = newJObject()
