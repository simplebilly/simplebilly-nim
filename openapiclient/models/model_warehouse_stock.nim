...
type WarehouseStock* = object
  ## 
  batchNumber*: Option[string] ## Batch/lot number (Chargennummer) — `None` for non-batched goods.
  binLocation*: Option[string]
  expiryDate*: Option[string] ## Expiry date for batch-tracked goods.
  productId*: string
  quantity*: int64
  serialNumbers*: Option[JsonNode] ## JSON array of serial numbers (Seriennummern) in this stock row.
  warehouseId*: string
