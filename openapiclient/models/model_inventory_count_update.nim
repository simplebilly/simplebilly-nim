#
# SimpleBilly API
# 
# Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
# The version of the OpenAPI document: 0.1.0
# Contact: support@simplebilly.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options

import model_any_type
import model_inventory_count_status

type InventoryCountUpdate* = object
  ## 
  countDate*: Option[string]
  countNumber*: Option[string]
  lineItems*: Option[JsonNode] ## JSON array of `{product_id, name, sku, expected_quantity, counted_quantity, bin_location?, batch_number?, variance}`.
  notes*: Option[string]
  status*: Option[InventoryCountStatus] ## One of: draft | counting | reviewed | posted
  warehouseId*: Option[string] ## References the warehouse entity.

