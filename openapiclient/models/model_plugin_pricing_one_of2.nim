#
# SimpleBilly API
# 
# Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
# The version of the OpenAPI document: 0.1.0
# Contact: support@simplebilly.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options


type `Type`* {.pure.} = enum
  Recurring

type PluginPricingOneOf2* = object
  ## 
  pricePerMonth*: float64
  `type`*: `Type`

func `%`*(v: `Type`): JsonNode =
  result = case v:
    of `Type`.Recurring: %"recurring"
func `$`*(v: `Type`): string =
  result = case v:
    of `Type`.Recurring: $("recurring")

proc to*(node: JsonNode, T: typedesc[`Type`]): `Type` =
  if node.kind != JString:
    raise newException(ValueError, "Expected string for enum `Type`, got " & $node.kind)
  let strVal = node.getStr()
  case strVal:
  of $("recurring"):
    return `Type`.Recurring
  else:
    raise newException(ValueError, "Invalid enum value for `Type`: " & strVal)


# Custom JSON deserialization for PluginPricingOneOf2 with custom field names
proc to*(node: JsonNode, T: typedesc[PluginPricingOneOf2]): PluginPricingOneOf2 =
  result = PluginPricingOneOf2()
  if node.kind == JObject:
    if node.hasKey("price_per_month"):
      result.pricePerMonth = to(node["price_per_month"], float64)
    if node.hasKey("type"):
      result.`type` = to(node["type"], `Type`)

# Custom JSON serialization for PluginPricingOneOf2 with custom field names
proc `%`*(obj: PluginPricingOneOf2): JsonNode =
  result = newJObject()
  result["price_per_month"] = %obj.pricePerMonth
  result["type"] = %obj.`type`

