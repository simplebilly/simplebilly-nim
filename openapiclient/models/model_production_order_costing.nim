#
# SimpleBilly API
# 
# Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
# The version of the OpenAPI document: 0.1.0
# Contact: support@simplebilly.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options

import model_costing_line

type ProductionOrderCosting* = object
  ## Actual-costing (Nachkalkulation) report for a production order.
  costPerUnit*: string ## material_cost_total ÷ quantity.
  costSource*: string ## \"actual\" when costed from stock-movement consumption, else \"planned\".
  lines*: seq[CostingLine]
  marginPerUnit*: Option[string] ## sale_price − cost_per_unit.
  marginPercent*: Option[string] ## margin_per_unit ÷ cost_per_unit as a percentage.
  materialCostTotal*: string ## Total material cost for the whole order.
  orderNumber*: string
  productionOrderId*: string
  quantity*: int64
  salePrice*: Option[string] ## Finished product's sale price per unit (used to compute margin).
  status*: string

