...
func `%`*(v: ConnectorType): JsonNode =
  result = case v:
    of ConnectorType.Apikey: %"apikey"
    of ConnectorType.Oauth: %"oauth"
...
proc to*(node: JsonNode, T: typedesc[ConnectorType]): ConnectorType =
  if node.kind != JString:
    raise newException(ValueError, "Expected string for enum ConnectorType, got " & $node.kind)
  let strVal = node.getStr()
  case strVal:
  of $("apikey"):
    return ConnectorType.Apikey
  of $("oauth"):
    return ConnectorType.Oauth
  else:
    raise newException(ValueError, "Invalid enum value for ConnectorType: " & strVal)
