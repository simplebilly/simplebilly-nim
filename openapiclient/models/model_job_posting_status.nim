#
# SimpleBilly API
# 
# Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
# The version of the OpenAPI document: 0.1.0
# Contact: support@simplebilly.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options


type JobPostingStatus* {.pure.} = enum
  Draft
  Published
  Closed

func `%`*(v: JobPostingStatus): JsonNode =
  result = case v:
    of JobPostingStatus.Draft: %"draft"
    of JobPostingStatus.Published: %"published"
    of JobPostingStatus.Closed: %"closed"

func `$`*(v: JobPostingStatus): string =
  result = case v:
    of JobPostingStatus.Draft: $("draft")
    of JobPostingStatus.Published: $("published")
    of JobPostingStatus.Closed: $("closed")

proc to*(node: JsonNode, T: typedesc[JobPostingStatus]): JobPostingStatus =
  if node.kind != JString:
    raise newException(ValueError, "Expected string for enum JobPostingStatus, got " & $node.kind)
  let strVal = node.getStr()
  case strVal:
  of $("draft"):
    return JobPostingStatus.Draft
  of $("published"):
    return JobPostingStatus.Published
  of $("closed"):
    return JobPostingStatus.Closed
  else:
    raise newException(ValueError, "Invalid enum value for JobPostingStatus: " & strVal)

