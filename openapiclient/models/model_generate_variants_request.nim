...
type GenerateVariantsRequest* = object
  ## Request body for generating the full variant set from option values.
  options*: Option[Table[string, seq[string]]] ## Option name → list of values, e.g. `{\"Color\": [\"Red\", \"Blue\"], \"Size\": [\"S\", \"M\"]}`. The cartesian product of these lists is generated.
  priceDelta*: Option[string] ## Optional per-variant price delta applied to every generated variant.
  productId*: string
  skuPrefix*: Option[string] ## Optional prefix for the generated SKUs (suffix is the option values joined by `-`). Falls back to the parent product's SKU.
