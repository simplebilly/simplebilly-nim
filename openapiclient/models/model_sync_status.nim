#
# SimpleBilly API
# 
# Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
# The version of the OpenAPI document: 0.1.0
# Contact: support@simplebilly.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options


type SyncStatus* {.pure.} = enum
  Pending
  Connected
  Success
  Error

func `%`*(v: SyncStatus): JsonNode =
  result = case v:
    of SyncStatus.Pending: %"pending"
    of SyncStatus.Connected: %"connected"
    of SyncStatus.Success: %"success"
    of SyncStatus.Error: %"error"

func `$`*(v: SyncStatus): string =
  result = case v:
    of SyncStatus.Pending: $("pending")
    of SyncStatus.Connected: $("connected")
    of SyncStatus.Success: $("success")
    of SyncStatus.Error: $("error")

proc to*(node: JsonNode, T: typedesc[SyncStatus]): SyncStatus =
  if node.kind != JString:
    raise newException(ValueError, "Expected string for enum SyncStatus, got " & $node.kind)
  let strVal = node.getStr()
  case strVal:
  of $("pending"):
    return SyncStatus.Pending
  of $("connected"):
    return SyncStatus.Connected
  of $("success"):
    return SyncStatus.Success
  of $("error"):
    return SyncStatus.Error
  else:
    raise newException(ValueError, "Invalid enum value for SyncStatus: " & strVal)

